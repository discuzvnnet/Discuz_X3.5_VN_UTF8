...
function loadcalendar() {
	...
	s += '<th colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');_cancelBubble(event)" title="Bấm chọn năm" id="year"></a>&nbsp; - &nbsp;<a id="month" title="Bấm để chọn một tháng" href="###" onclick="showdiv(\'month\');_cancelBubble(event)"></a></th>';
	s += '<th><A href="###" onclick="refreshcalendar(yy, mm+1)" title="Tháng tiếp theo">》</A></th>';
	s += '</tr>';
	...
	s += '<tr id="hourminute"><td colspan="7" align="center"><input type="text" size="2" value="" id="hour" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> : <input type="text" size="2" value="" id="minute" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> phút</td></tr>';
	s += '</tbody>';
	s += '</table></div></div>';
	...
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\'; ">tháng<span' + (today.getMonth()+1 == k ? ' class="bold"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' </span></a><br />';
	}
