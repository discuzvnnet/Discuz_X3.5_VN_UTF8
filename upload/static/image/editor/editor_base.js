...
function renewContent() {
	...
	if(window.confirm('Bạn có chắc chắn muốn khôi phục lần lưu cuối cùng không?')) {
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "Cài đặt bảo mật của trình duyệt của bạn không cho phép trình chỉnh sửa tự động thực hiện thao tác cắt, vui lòng sử dụng phím tắt (Ctrl + X) để hoàn thành";
				break;
			case "Copy":
				sAlert = "Cài đặt bảo mật của trình duyệt của bạn không cho phép trình chỉnh sửa tự động thực hiện thao tác sao chép, vui lòng sử dụng phím tắt (Ctrl + C) để hoàn thành";
				break;
			case "Paste":
				sAlert = "Cài đặt bảo mật trình duyệt của bạn không cho phép trình chỉnh sửa tự động thực hiện thao tác dán, vui lòng sử dụng phím tắt (Ctrl + V) để hoàn thành";
				break;
		}
	}
	if(sAlert != ""){
...
function doodleBox(event, id) {
	if(parent.$('uchome-ttHtmlEditor') != null) {
		...
		alert("Không tìm thấy dữ liệu trong bản vẽ");
	}
}
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
		alert('Chỉ hỗ trợ IE 5.01 trở lên');
		return;
	}
	fHideMenu();
...
function fSetModeTip(obj){
	...
	if(!dvModeTip){
		...
		dv.innerHTML = "Chỉnh sửa mã nguồn";
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm("Một số định dạng sẽ bị mất khi chuyển đổi sang văn bản thuần túy.\nBạn có chắc muốn tiếp tục?")){
...
function changeEditFull(flag, ev) {
	if(parent.changeEditFull) {
		...
		ele.innerHTML = flag ? 'Trở về' : 'Toàn màn hình';
