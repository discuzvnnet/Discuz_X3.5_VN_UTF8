...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? 'Bạn có chắc chắn muốn xóa dữ liệu này không?' : 'Bạn có chắc chắn muốn chặn dữ liệu này không?';
	if(confirm(msg)) {
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo;Trở lại</em>';
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('ID mô-đun không được để trống', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('Lỗi nội dung tùy chỉnh, mã HTML: '+tag+' không khớp', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('Mã HTML: '+tag+' không khớp', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
