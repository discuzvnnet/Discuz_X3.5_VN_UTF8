...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		leftmsg = locationtime + ' giây sau sẽ chuyển trang';
...
function setCopy(text, msg) {
	...
	if(success) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Sao chép không thành công, vui lòng chọn “Cho phép truy cập”</div></div>', 'alert');
		}
	} else {
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '[]' : '[Lời nhắc mới]') + NOTICETITLE['oldTitle'];
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("Vui lòng nhấn Ctrl+D để thêm vào mục yêu thích", 'notice');
		}
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'Bật truy cập được hỗ trợ';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'Tắt quyền truy cập hỗ trợ';
		}
	}
}
