	...
	x.getJSON(url+'&_makehtml&r='+(+ new Date()), function(ret){
		...
		if(ret && (ret=ret['data']) && ret['status'] == 'html_ok') {
			if(obj) {
				...
				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+' được tạo thành công</a>';
			}
			if(ret['nexturl']) {
				if(obj) {
			...
			if(obj) {
				obj.style.color = 'red';
				obj.innerHTML = title+' không thể tạo';
			}
		}
	});
	...
	make : function (id, child) {
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			...
			if(obj.single) {
				...
				progress_bar(cent);
				var str = 'Tổng số '+obj.count+' tệp '+obj.dom.getAttribute('mktitle')+' cần được tạo '+obj.makedcount+' lần, ';
				if(cent != 100) {
					child.innerHTML = str+' đang tạo '+current+' lần, đã hoàn thành '+cent+'%';
				} else {
					child.style.color = 'blue';
					child.innerHTML = str+obj.dom.getAttribute('mktitle')+' được tạo ra';
				}
				if(id) {
					obj.make(id, child);
