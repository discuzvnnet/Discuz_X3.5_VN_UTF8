...
function removeindexheats() {
	return confirm('Bạn có chắc chắn muốn xóa chủ đề này khỏi các chủ đề phổ biến không?');
}
...
function fastpostvalidate(theform, noajaxpost) {
	...
	if(theform.message.value == '' || theform.subject.value == '') {
		s = 'Xin lỗi, bạn chưa nhập tiêu đề hoặc nội dung';
	...
	if(!disablepostctrl && dstrlen(trim(theform.subject.value)) && ((postminsubjectchars != 0 && dstrlen(theform.subject.value) < postminsubjectchars) || (postminsubjectchars != 0 && dstrlen(theform.subject.value) > postmaxsubjectchars))) {
		showError('Tiêu đề của bạn không đáp ứng độ dài yêu cầu.\n\nĐộ dài hiện tại: ' + dstrlen(theform.subject.value) + ' từ\nGiới hạn hệ thống: ' + postminsubjectchars + ' đến ' + postmaxsubjectchars + ' từ');
		return false;
	}
	if(!disablepostctrl && ((postminchars != 0 && mb_strlen(theform.message.value) < postminchars) || (postmaxchars != 0 && mb_strlen(theform.message.value) > postmaxchars))) {
		s = 'Bài viết của bạn không đủ độ dài theo yêu cầu.\n\nĐộ dài hiện tại: ' + mb_strlen(theform.message.value) + ' ' + ' bytes\nGiới hạn hệ thống: ' + postminchars + ' đến ' + postmaxchars + ' bytes';
	}
	if(s) {
		showError(s);
		doane();
...
function loadData(quiet, formobj) {
	...
	if(!quiet && !confirm('Thao tác này sẽ ghi đè lên nội dung của bài đăng hiện tại, bạn có chắc chắn muốn khôi phục dữ liệu không?')) {
		return;
	}
...
function checkForumnew(fid, lasttime) {
	...
	x.get('forum.php?mod=ajax&action=forumchecknew&fid=' + fid + '&time=' + lasttime + '&inajax=yes', function(s){
		if(s > 0) {
			...
			var checknew = {'tid':'', 'thread':{'common':{'className':'', 'val':'<a href="javascript:void(0);" onclick="ajaxget(\'forum.php?mod=ajax&action=forumchecknew&fid=' + fid+ '&time='+lasttime+'&uncheck=1&inajax=yes\', \'forumnew\');">Chủ đề có câu trả lời mới, bấm vào để xem', 'colspan': colspan }}};
