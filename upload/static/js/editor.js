...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'Toàn màn hình' : 'Trở lại';
				...
				buttons[i].innerHTML = !simplodemode ? 'Thông thường' : 'Nâng cao';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('Trình duyệt của bạn không hỗ trợ tính năng này, vui lòng nâng cấp phiên bản trình duyệt của bạn', 'notice', 'Nhắc nhở thân thiện');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Nhấp để bật lưu tự động" href="javascript:;" onclick="setAutosave()">Bật lưu tự động</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('Dữ liệu được lưu tại ' + h + ':' + m + '');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="Bấm để tắt tự động lưu" href="javascript:;" onclick="setAutosave()">Lưu sau ' + savedatac + ' giây</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Tự động lưu dữ liệu đang bật' : 'Tự động lưu dữ liệu đã tắt');
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'Thông thường';
...
function checklength(theform) {
	...
	showDialog('Độ dài hiện tại: ' + mb_strlen(message) + ' byte, ' + (postmaxchars != 0 ? 'Giới hạn hệ thống:' + postminchars + ' tới ' + postmaxchars + ' byte' : ''), 'notice', 'Kiểm tra số lượng từ');
}
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'Vui lòng nhập địa chỉ liên kết:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
			...
			case 'free':
				...
				var lang = {'quote' : 'Vui lòng nhập trích dẫn để chèn', 'code' : 'Vui lòng nhập mã để chèn', 'hide' : 'Vui lòng nhập nội dung thông tin cần ẩn', 'free' : 'Nếu bạn đặt giá bán sau, vui lòng nhập nội dung thông tin miễn phí và hiển thị trước khi mua'};
				...
				break;
			case 'tbl':
				str = '<p class="pbn">Hàng của bảng: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Số cột bảng: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Chiều rộng biểu mẫu: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Màu nền: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Gợi ý\', null, 0)"><img id="tbltips" title="Gợi ý" class="vm" src="' + IMGDIR + '/info_small.gif"> Mẹo để viết biểu mẫu một cách nhanh chóng</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=color]” xác định nền hàng<br />“[td=width]” xác định độ rộng của cột<br />“[td=column span, row span, width]” xác định khoảng cách của hàng và cột<br /><br />Ví dụ về bảng viết nhanh:<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>Sử dụng “|” để phân tách từng cột. Nếu có “|” trong bảng, hãy sử dụng “\\|” để thay thế và sử dụng “\\n” để thay thế ngắt dòng.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Vui lòng nhập địa chỉ file nhạc:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Hỗ trợ các định dạng nhạc như wma mp3 ra rm<br />Ví dụ: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">Vui lòng nhập địa chỉ video:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Chiều rộng: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; Chiều cao: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Hỗ trợ URL video của Youku, Tudou, 56, Ku6 và các trang video khác<br />Hỗ trợ wmv avi rmvb mov swf flv và các định dạng video khác<br />Ví dụ: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">Vui lòng nhập URL của tệp Flash:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">Chiều rộng: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; Chiều cao: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">Hỗ trợ các URL flash như swf flv<br />Ví dụ: http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">Vui lòng nhập Flash hoạt hình mở đầu hoặc địa chỉ hình ảnh:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Bấm vào địa chỉ liên kết:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				...
				str += '<p class="pbn">Ở lại giây: <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Hiệu ứng tải và biến mất: </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />Không có &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />Tăng giảm dần &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />mở rộng đã đóng</p>';
				str += '<p class="xg2 pbn">Hỗ trợ swf flv jpg gif png URL<br />Phạm vi chiều rộng và chiều cao: chiều rộng 400~1024 chiều cao 300~640<br />Ví dụ: http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'Dán nội dung từ Word';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">Vui lòng sử dụng tổ hợp phím tắt (Ctrl+V) để dán nội dung của file Word bên trên</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				stitle = 'Tạo thư mục bài viết';
				str = '<p class="pbn">[index]<br />\n\
					...
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">Tiêu đề</span> &nbsp;&nbsp;<span class="xg1">Chuyển tới bài đăng được chỉ định</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">Số trang</span> &nbsp;&nbsp;<span class="xg1">[page] Số trang hiện tại của bài đăng.</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">TID và PID của bài đăng</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">Thêm thụt vào đầu dòng</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'Vui lòng nhập số ' + i + ':') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
