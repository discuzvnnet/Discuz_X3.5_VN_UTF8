...
var page = {
	converthtml : function() {
		...
		pgobj.innerHTML = '<a href="'+ prevpagehref +'">Trang trước</a>'+ selector +'<a href="'+ nextpagehref +'">Trang tiếp</a>';
...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">Nhấp để tải lại</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Chắc chắn" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><a class="button" href="'+ url +'">Chắc chắn</a> <button onclick="popup.close();" class="button">Hủy bỏ</a></dd></div>'
			}
...
var formdialog = {
	init : function() {
		$(document).on('click', '.formdialog', function() {
			...
			.error(function() {
				popup.open('Ngoại lệ gửi biểu mẫu, không thể hoàn thành yêu cầu của bạn', 'alert');
			});
			return false;
		});
	}
