...
var STATUSMSG = {
	...
	'1' : 'Loại tiện ích mở rộng này không được hỗ trợ',
	'2' : 'Giới hạn máy chủ không thể tải lên tệp đính kèm lớn như vậy',
	'3' : 'Hạn chế nhóm người dùng không thể tải lên tệp đính kèm lớn như vậy',
	'4' : 'Loại tiện ích mở rộng này không được hỗ trợ',
	'5' : 'Hạn chế loại tệp không thể tải lên tệp đính kèm lớn như vậy',
	'6' : 'Hôm nay bạn không thể tải lên bất kỳ tệp đính kèm nào nữa',
	'7' : 'Vui lòng chọn một tệp hình ảnh (' + imgexts + ')',
	'8' : 'Không thể lưu tệp đính kèm',
	'9' : 'Không có tệp hợp lệ nào được tải lên',
	'10' : 'Hoạt động bất hợp pháp',
	'11' : 'Bạn không còn có thể tải lên các tệp đính kèm có dung lượng lớn như hiện nay',
	'12' : 'Không gửi được vì tên file chứa từ nhạy cảm',
	'13' : 'Hạn chế server không upload được file đính kèm có độ phân giải quá cao'
};
...
function checklength(theform) {
	...
	showDialog('Độ dài hiện tại: ' + mb_strlen(message) + ' byte, ' + (postmaxchars != 0 ? 'giới hạn hệ thống: ' + postminchars + ' đến ' + postmaxchars + ' byte.' : ''), 'notice', 'Kiểm tra số lượng từ');
}
...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('Xin lỗi, bạn chưa nhập tiêu đề hoặc nội dung');
		return false;
	} else if(dstrlen(theform.subject.value) > 255) {
		showError('Tiêu đề của bạn vượt quá giới hạn 255 ký tự');
		return false;
	}
	if(!disablepostctrl && theform.subject.value != "" && ((postminsubjectchars != 0 && dstrlen(theform.subject.value) < postminsubjectchars) || (postminsubjectchars != 0 && dstrlen(theform.subject.value) > postmaxsubjectchars))) {
		showError('Tiêu đề của bạn không đáp ứng độ dài yêu cầu.\n\nChiều dài hiện tại: ' + dstrlen(theform.subject.value) + ' từ\nHạn chế hệ thống: ' + postminsubjectchars + ' đến ' + postmaxsubjectchars + ' byte');
		return false;
	}
	if(in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
		if(theform.typeid && (theform.typeid.options && theform.typeid.options[theform.typeid.selectedIndex].value == 0) && typerequired) {
			showError('Vui lòng chọn danh mục tương ứng với chủ đề');
			return false;
		}
		if(theform.sortid && (theform.sortid.options && theform.sortid.options[theform.sortid.selectedIndex].value == 0) && sortrequired) {
			showError('Vui lòng chọn thông tin phân loại tương ứng với chủ đề');
			return false;
		}
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('Bài viết của bạn không đủ độ dài theo yêu cầu.\n\nĐộ dài hiện tại: ' + mb_strlen(message) + ' byte\nGiới hạn hệ thống:  ' + postminchars + ' đến ' + postmaxchars + ' byte');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('Bạn có tệp đính kèm đang chờ tải lên, bạn có chắc chắn không tải chúng lên không?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('Bạn có một tệp đính kèm đang được tải lên, vui lòng đợi trong giây lát, bài đăng sẽ được xuất bản tự động sau khi tải lên hoàn tất...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('Vì bạn đã đặt quyền đọc hoặc bán bài đăng, bạn có chắc chắn sẽ phát lại bài đăng đó cho khán giả của mình không?') == false) {
			return false;
		}
	}
			...
			if(secqaacheck) {
				...
				if(chkv.indexOf('loading') != -1) {
					...
					if($('secqaaverify_' + theform.secqaahash.value) == document.activeElement) {
						...
						showError('Lỗi câu hỏi và câu trả lời xác minh, vui lòng điền lại');
					}
					chk = 0;
				}
			}
			if(seccodecheck) {
				...
				if(chkv.indexOf('loading') !== -1) {
					...
					if($('seccodeverify_' + theform.seccodehash.value) == document.activeElement) {
						...
						showError('Lỗi mã xác minh, vui lòng nhập lại');
					}
					chk = 0;
				}
			}
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(Loại tệp đính kèm bị cấm)';
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('Tải lên tệp đính kèm hoàn tất! ' + UPLOADCOMPLETE + ' thành công, ' + UPLOADFAILED + ' thất bại:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('Tải lên tệp đính kèm hoàn tất!', 'right', null, null, 0, null, null, null, null, 3);
			}
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<div class="loadicon" title="Đang tải lên..."></div>';
	UPLOADSTATUS = 1;
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('Rất tiếc, không hỗ trợ tải lên tệp đính kèm bằng tiện ích mở rộng này.');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('Vui lòng chọn một tệp hình ảnh (' + imgexts + ')');
		return;
	}
...
function appendAttachDel(ids) {
	...
	if(formhash) {
		...
		showError('Rất tiếc, thao tác xóa không thành công, vui lòng làm mới trang và thử lại.');
	}
}
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">Hủy bỏ</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = 'Chứa ' + num + (type == 'image' ? ' tệp đính kèm hình ảnh' : ' tập tin đính kèm');
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? 'hình ảnh' : 'tệp đính kèm';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		showError('Xin lỗi, không có tệp đính kèm từ xa');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog('Đã hoàn tất tải xuống tệp đính kèm từ xa!', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(Không thể vượt quá ' + arr[8] + ' byte)';
		} else if(arr[7] > 0) {
			sizelimit = '(Không thể vượt quá ' + arr[7] + ' byte)';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').innerHTML = 'Số phiếu bầu tối đa đã đạt ' + maxoptions;
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />Chọn tất cả</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>Sử dụng</strong></button></p>', 'info', 'Không sử dụng' + (type == 'attach' ? 'tập tin đính kèm' : 'hình ảnh'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">Trả lời tổng điểm thưởng quá lớn ('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>Trả lại "+Math.abs(real_reply_credit)+"</font>";
		} else {
