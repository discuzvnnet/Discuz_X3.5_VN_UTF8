...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'Đã sao chép mã vào khay nhớ tạm');
}
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Mở trong cửa sổ mới">Mở trong cửa sổ mới</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Kích thước thực sự">Kích thước thực sự</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Đóng">Đóng</a></span>Con lăn chuột zoom ảnh</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">Đóng</a></span>Con lăn chuột zoom ảnh<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!$(ctrlid + '_menu')) {
		...
		var colortexts = ['đen', 'nâu', 'xanh ô liu', 'xanh lá', 'xanh xám', 'xanh hải quân', 'xanh đậm', 'xám nhạt', 'đỏ sẫm', 'cam đậm', 'ô liu', 'xanh lá', 'mòng két', 'xanh da trời', 'đá tro', 'xám đen', 'đỏ', 'nâu cát', 'vàng xanh', 'xanh nước biển', 'ngọc lục bảo', 'xanh đậm', 'tím', 'xám', 'đỏ tươi', 'cam', 'vàng', 'cam chanh', 'xanh thẳm', 'xanh thẳm', 'tím đậm', 'bạc', 'hồng', 'vàng nhạt', 'kem', 'xanh nhạt', 'xanh ngọc', 'xanh sáng', 'tím nhạt', 'trắng'];
		var str = '';
		for(var i = 0; i < 40; i++) {
...
function _widthauto(obj) {
	var strs = ['Chuyển sang rộng', 'Chuyển sang thu hẹp'];
	if($('css_widthauto')) {
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1. Nhấp để "' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>tải xuống lối tắt trên màn hình</a>' + '". Sau khi tải xuống hoàn tất, bạn có thể di chuyển tệp vào màn hình hệ thống<br />';
		msg += '2. Nhấp để "' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'tải xuống biểu tượng ICO</a>' + '". Sau khi tải xuống hoàn tất, nhấp chuột phải vào tệp lối tắt trên màn hình->Thuộc tính->Thay đổi biểu tượng và chọn biểu tượng ICO đã tải xuống.';
