...
function fileQueueError(errorCode) {
	try {
		...
		switch (errorCode) {
		case 'F_EXCEED_SIZE':
			err = 'Kích thước tệp riêng lẻ không được vượt quá ' + WebUploader.Base.formatSize(this.uploader.option('fileSingleSizeLimit')) + '';
			break;
		case 'Q_EXCEED_NUM_LIMIT':
			err = 'Bạn chỉ có thể tải lên tối đa ' + this.settings.fileNumLimit + ' tệp';
			break;
		case 'Q_EXCEED_SIZE_LIMIT':
			err = 'Tổng kích thước tệp tải lên vượt quá ' + WebUploader.Base.formatSize(this.uploader.option('fileSizeLimit')) + '';
			break;
		case 'Q_TYPE_DENIED':
			err = 'Loại tệp không hợp lệ, vui lòng tải lên đúng định dạng tệp!';
			break;
		case 'F_DUPLICATE':
			err = 'Vui lòng không tải lên cùng một tệp nhiều lần!';
			break;
		default:
...
function uploadProgress(file, percentage) {
	try {
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("Đang tải lên <progress value='" + percentage + "' max='1' style='width: 200px;'></progress> " + Math.ceil(percentage * 100) + "%");
	} catch (ex) {
		this.debug(ex);
	}
}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			...
			progress.setStatus("Hoàn tất tải lên.");
