...
function extrafunc_atListMenu(tag, op) {
	...
	if(!op) {
		...
		return 'Vui lòng nhập tên người dùng:<br /><input type="text" id="atkeyword" style="width:240px" value="" class="px" onkeydown="atEnter(event, \'atListSet\')" onkeyup="atFilter(this.value, \'at_list\',\'atListSet\',event, true);" /><div class="p_pop" id="at_list" style="width:250px;"><ul><li>@tài khoản bạn bè, bạn có thể nhắc anh ấy đọc bài đăng</li></ul></div>';
	} else {
		if($('atkeyword').value) {
...
function atMenu(x, y) {
	...
	$('at_menu').innerHTML = '<div class="loadicon vm"></div> Vui lòng chờ...';
}
...
function atFilter(kw, id, call, e, nae) {
	...
	if(nae || !atEnter(e, call)) {
		...
		if(atResult.length) {
			...
			$(id).innerHTML = '<ul>' + newlist + '<li class="xg1">@tài khoản bạn bè, bạn có thể nhắc anh ấy đọc bài đăng</li></ul>';
		} else {
