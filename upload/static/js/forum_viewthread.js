		...
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				imagelist += '<div class="pattimg">' +
					'<a id="pattimg_' + aimgcount[pid][i] + '" class="pattimg_zoom" href="javascript:;"' + s + ' onclick="zoom($(\'aimg_' + aimgcount[pid][i] + '\'), attachimggetsrc(\'aimg_' + aimgcount[pid][i] + '\'), 0, 0, ' + (parseInt(showexif) ? 1 : 0) + ')" title="Nhấp để phóng to">Nhấp để phóng to</a>' +
...
function succeedhandle_fastpost(locationhref, message, param) {
	...
	if(!reply_mod) {
		...
		if(!message) {
			message = 'Phiên bản trả lời này cần được xem xét và bài đăng của bạn sẽ được hiển thị sau khi vượt qua bài đánh giá';
		}
...
function copyThreadUrl(obj, bbname) {
	...
	setCopy($('thread_subject').innerHTML.replace(/&amp;/g, '&') + '\n' + obj.href + '\n' + '(Nguồn: '+bbname+')' + '\n', 'Địa chỉ bài đăng đã được sao chép vào khay nhớ tạm');
	return false;
}
...
function show_threadpage(pid, current, maxpage, ispreview) {
	...
	if(current < maxpage) {
		s += '<a href="javascript:;" onclick="' + clickvalue(current + 1) + '" class="nxt">Trang tiếp</a>';
	}
...
function show_threadindex(pid, ispreview) {
	if(!show_threadindex_data) {
		var s = '<div class="tindex"><h3>Mục lục</h3><ul>';
...
function showmobilebbs(obj) {
	var content = '<h3 class="flb" style="cursor:move;"><em>Tải xuống diễn đàn bỏ túi</em><span><a href="javascript:;" class="flbc" onclick="hideWindow(\'mobilebbs\')" title="{lang close}">{lang close}</a></span></h3><div class="c"><h4>Phiên bản Android, quét mã QR để tải trực tiếp về điện thoại</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_andriod.png" alt="" /></span><img src="'+ STATICURL +'image/common/andriod.png" alt="Áp dụng cho điện thoại di động Samsung/HTC/Xiaomi với hệ thống Android" /></p><h4>Phiên bản iPhone, quét mã QR để tải trực tiếp về điện thoại</h4><p class="mtm mbm vm"><span class="code_bg"><img src="'+ STATICURL +'image/common/zslt_ios.png" alt="" /></span><img src="'+ STATICURL +'image/common/ios.png" alt="Áp dụng cho iPhone" /></p></div>';
