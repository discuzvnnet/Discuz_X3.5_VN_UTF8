...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'Chọn kiểu','value':''},{'key':'Không viền và không có lề','value':'cl_block_bm'},{'key':'Kiểu 1','value':'xbs_1'},{'key':'Kiểu 2','value':'xbs xbs_2'},{'key':'Kiểu 3','value':'xbs xbs_3'},{'key':'Kiểu 4','value':'xbs xbs_4'},{'key':'Kiểu 5','value':'xbs xbs_5'},{'key':'Kiểu 6','value':'xbs xbs_6'},{'key':'Kiểu 7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'Chọn kiểu','value':''},{'key':'Không viền và không có lề','value':'cl_frame_bm'},{'key':'khung không viền','value':'xfs xfs_nbd'},{'key':'Kiểu 1','value':'xfs xfs_1'},{'key':'Kiểu 2','value':'xfs xfs_2'},{'key':'Kiểu 3','value':'xfs xfs_3'},{'key':'Kiểu 4','value':'xfs xfs_4'},{'key':'Kiểu 5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		this.addMenu('default','Tiêu đề','drag.openTitleEdit(event)');
		this.addMenu('default','Kiểu','drag.openStyleEdit(event)');
		this.addMenu('default', 'Xóa', 'drag.removeBlock(event)');
		this.addMenu('block', 'Thuộc tính', 'drag.openBlockEdit(event)');
		this.addMenu('block', 'Dữ liệu', 'drag.openBlockEdit(event,"data")');
		this.addMenu('block', 'Cập nhật', 'drag.blockForceUpdate(event)');
		this.addMenu('frame', 'Xuất', 'drag.frameExport(event)');
		this.addMenu('tab', 'Xuất', 'drag.frameExport(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'Gạch','value':'repeat'},{'key':'Không gạch','value':'no-repeat'},{'key':'Gạch ngang','value':'repeat-x'},{'key':'Gạch dọc','value':'repeat-y'}];
		var stylearr = [{'key':'Không kiểu','value':'none'},{'key':'Nét liền','value':'solid'},{'key':'Điểm','value':'dotted'},{'key':'Đường chấm','value':'dashed'}];
		...
		table += '<tr><th>Khung</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>Trên</label><select class="ps vm" id="bdtwidth" ><option value="">Kích cỡ</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		table += ' <select class="ps vm" id="bdtstyle" ><option value="">Kiểu</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
		table += ' Màu sắc <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		table += getColorPalette(id+'_bdtPalette', 'bdtcolor' ,bdtcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Phải</label><select class="ps vm" id="bdrwidth" ><option value="">Kích cỡ</option>'+this.getOption(widtharr,bdrwidth)+'</select>';
		table += ' <select class="ps vm" id="bdrstyle" ><option value="">Kiểu</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
		table += ' Màu sắc <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		table += getColorPalette(id+'_bdrPalette', 'bdrcolor' ,bdrcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Dưới</label><select class="ps vm" id="bdbwidth" ><option value="">Kích cỡ</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		table += ' <select class="ps vm" id="bdbstyle" ><option value="">Kiểu</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
		table += ' Màu sắc <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		table += getColorPalette(id+'_bdbPalette', 'bdbcolor' ,bdbcolor)+'</li>';

		table += '<li class="bordera mtn"><label>Trái</label><select class="ps vm" id="bdlwidth" ><option value="">Kích cỡ</option>'+this.getOption(widtharr,bdlwidth)+'</select>';
		table += ' <select class="ps vm" id="bdlstyle" ><option value="">Kiểu</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
		table += ' Màu sắc <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		...
		table += '<tr><th>Lề</th><td><div id="margindiv" class="'+ulclass+'"><span><label>Trên</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '<span class="bordera"><label>Phải</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '<span class="bordera"><label>Dưới</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>Đệm</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>Trên</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Phải</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Dưới</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
		...
		table += '<tr><th>Hình nền</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>Chỉ định class</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';
		...
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>Chắc chắn</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>Hủy bỏ</strong></button></p>';
		this.removeMenu(e);
	...
	openTitleEdit : function (e) {
		...
		common += '<tr><th>Hình nền:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		...
		li += '<tr><th>Hình ảnh:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>Vị trí:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>Trái</option><option value="right" `right`>Phải</option></select>';
		li += '&nbsp;&nbsp;Độ lệch: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
		li += '<tr><th>Nét chữ:</th><td><select class="ps vm" id="titleSize_0" ><option value="">Kích cỡ</option>`size`</select>';
		...
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>Chắc chắn</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>Hủy bỏ</strong></button></p>';
		this.removeMenu(e);
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>Tải nội dung...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('Xin lỗi, bạn không có quyền thêm hoặc chỉnh sửa mô-đun', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('Bạn đã thực hiện các thay đổi, vui lòng lưu và sau đó xuất, nếu không dữ liệu được xuất sẽ không bao gồm các thay đổi bạn đã thực hiện lần này.');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'Thao tác hoàn tất.';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'Tổng cộng có <span style="color:blue">'+this.blocksLen+'</span> mô-đun, đang cập nhật <span style="color:red">'+cur+'</span>, đã hoàn thành <span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	clearAll : function () {
		if (confirm('Bạn có thực sự muốn xóa dữ liệu DIY trên trang không, nó sẽ không thể phục hồi sau khi xóa')) {
	...
	getFrameHtml : function (type) {
		...
		str += '<div id="'+id+'_title" class="'+this.titleClass+' '+this.frameTitleClass+'"><span class="'+this.titleTextClass+'">Khung '+type+'</span></div>';
		var cols = type.split('-');
		var clsl='',clsc='',clsr='';
		clsl = ' frame-'+type+'-l';
		clsc = ' frame-'+type+'-c';
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">nhãn tab</span></div>';
		...
		if(this.sampleMode) {
			...
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'Dữ liệu của bạn đã được sửa đổi, thoát sẽ không lưu sửa đổi của bạn.';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">Nhấn nút tiếp tục sẽ mở dữ liệu tạm thời và DIY, <br />Nhấn nút xóa sẽ xóa dữ liệu tạm thời.</div>','confirm','Bạn có muốn tiếp tục tự làm lưu trữ dữ liệu tạm thời không?', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'Tiếp tục', 'Xóa bỏ');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />Áp dụng cho tất cả các trang thuộc loại này</label>\n\
	...
	cancel : function () {
		...
		if (drag.isClearClose) {
			showDialog('<div style="line-height:28px;">Bạn có muốn giữ dữ liệu dàn dựng không?<br />Nhấn nút OK sẽ giữ dữ liệu tạm thời và nhấn nút Hủy sẽ xóa dữ liệu tạm thời.</div>','confirm','Giữ dữ liệu tạm thời', function(){location.href = spaceDiy.cancelDiyUrl();}, true, function(){window.onunload=function(){spaceDiy.cancelDIY()};location.href = spaceDiy.cancelDiyUrl();});
		} else {
	...
	recover : function() {
		if (confirm('Bạn có chắc chắn muốn hoàn nguyên về kết quả đã lưu trong phiên bản trước không?')) {
			drag.clearClose();
