...
var lastusername = '', lastpassword = '', lastemail = '', lastinvitecode = '', stmp = new Array(), modifypwd = false, profileTips = 'Nếu bạn không muốn thay đổi mật khẩu, vui lòng để trống';
...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','Yếu','Trung','Mạnh');
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, 'Vui lòng nhập địa chỉ email');
		}
...
function checkusername(id) {
	...
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? 'Tên người dùng phải có ít nhất 3 ký tự' : 'Tên người dùng không được vượt quá 15 ký tự');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, 'Mật khẩu quá ngắn, phải có ít nhất '+pwlength+' ký tự');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			if(strongpw[i] === 1 && !$(id1).value.match(/\d+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Con số';
				j++;
			}
			if(strongpw[i] === 2 && !$(id1).value.match(/[a-z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Chữ viết thường';
				j++;
			}
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Chữ viết hoa';
				j++;
			}
			if(strongpw[i] === 4 && !$(id1).value.match(/[^A-Za-z0-9]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Ký hiệu đặc biệt';
				j++;
			}
		}
		if(strongpw_error) {
			errormessage(id1, 'Mật khẩu quá yếu, phải chứa '+strongpw_str.join('，'));
			return;
		}
	}
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'Hai mật khẩu đã nhập không khớp');
	} else {
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', 'Mã mời chứa các ký tự nhạy cảm');
		return;
	}
	var x = new Ajax();
